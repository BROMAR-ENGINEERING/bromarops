...
window.BromarPages.scheduling = (() => {
  ...
  const DB = {
    ...
    async fetchJobs() {
      try {
        ...
        const { data, error } = await sb.from('job_number_register')
          .select('job_number, client_name, site_name, site_address, status')
          .eq('status', 'active')
          .order('created_at', { ascending: false });
        if (error) {
          if (error.code === '42P01' || error.message?.includes('does not exist')) {
            console.warn('job_number_register table not found');
            return [];
          }
          throw error;
        }
        return (data || []).map(j => ({
          ...
          site: j.site_name || j.site_address || '',
    ...
    async fetchPinnedSites() {
      try {
        const sb = await this.init();
        const { data, error } = await sb.from('pinned_sites').select('*').order('site_name');
        if (error) { if (error.code === '42P01') return []; throw error; }
        return (data || []).map(p => ({ id: p.site_id, name: p.site_name, clientName: p.client_name || '', pinId: p.id }));
      } catch (err) { console.warn('Pinned fetch failed:', err.message); return []; }
    },

    async pinSite(site) {
      try {
        const sb = await this.init();
        const { error } = await sb.from('pinned_sites').insert({ site_id: site.id, site_name: site.name, client_name: site.clientName });
    ...
    async saveAssignment(a) {
      try {
        ...
        const row = {
          ...
          was_notified: a.wasNotified || false,
          is_active: true
        };
  ...
  function buildDefaultMessage(a) {
    ...
    // Location part: Site - JobNumber (whichever present)
    let loc = '';
    if (a.type==='workshop') loc = 'Workshop/Office';
    else if (a.type==='leave') loc = a.siteName || 'Leave';
    else {
      const bits = [];
      if (a.siteName) bits.push(a.siteName);
      if (a.jobNumber) bits.push(a.jobNumber);
      loc = bits.join(' - ');
    }
    let msg = loc ? `${day} - ${loc}` : day;
    const times = getAssignmentTimes(a);
    if (times) msg += ` @ ${times}`;
    if (a.schedule!=='oneoff') { const dateRef = a.endDate ? `${a.startDate} to ${a.endDate}` : `from ${a.startDate}`; msg += ` (${dateRef})`; }
    if (a.notes) msg += `\nNote: ${a.notes}`;
    return msg;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .sched-pin-btn{background:none;border:none;color:var(--text-secondary);font-size:1.1rem;cursor:pointer;padding:0 0.25rem;flex-shrink:0;transition:color 0.15s}
      .sched-pin-btn:hover{color:#fbbf24}
      .sched-pin-btn.pinned{color:#fbbf24}
      .sched-loc-toggle{display:flex;gap:0.4rem;margin-top:0.4rem}
      .sched-loc-btn{flex:1;padding:0.5rem;border-radius:8px;border:1px solid var(--border);background:var(--bg-main);font-family:'Outfit',sans-serif;font-size:0.78rem;font-weight:600;color:var(--text-secondary);cursor:pointer;transition:all 0.15s}
      .sched-loc-btn:hover{border-color:var(--accent);color:var(--text-primary)}
      .sched-loc-btn.active{border-color:var(--accent);background:var(--card-hover);color:var(--accent)}
      .sched-job-card .job-loc-badge{font-size:0.55rem;font-weight:700;color:#f59e0b;background:#f59e0b18;padding:0.05rem 0.3rem;border-radius:3px;align-self:flex-start;margin-top:1px}
      .sched-job-card .job-notes{font-size:0.6rem;color:var(--text-secondary);font-style:italic;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px}
      ...
      .sched-legend{display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:1rem;padding:0.6rem 0.75rem;border-radius:var(--radius);background:var(--bg-secondary);border:1px solid var(--border)}
      .sched-legend-items{display:flex;gap:1rem;flex-wrap:wrap}
      .sched-legend .lg-item{display:flex;align-items:center;gap:0.4rem;font-size:0.75rem;color:var(--text-secondary);font-weight:500}
      .sched-legend .lg-dot{width:12px;height:12px;border-radius:50%;flex-shrink:0}
      .sched-job-card.jstatus-red{border-left-color:var(--error)!important}
      .sched-job-card.jstatus-amber{border-left-color:#f59e0b!important}
      .sched-job-card.jstatus-green{border-left-color:var(--success)!important}
      .sched-job-card.jstatus-blue{border-left-color:#3b82f6!important}
      .sched-jobtile.status-blue{border-left-color:#3b82f6}
      .sched-jobtile.status-blue .jt-dot{background:#3b82f6}
      ...
      .sched-job-rail{width:200px;flex-shrink:0;background:var(--bg-secondary);border:1px solid var(--border);border-radius:var(--radius);padding:0.75rem;max-height:70vh;overflow-y:auto;transition:outline 0.15s}
      .sched-job-rail.rail-drop{outline:2px dashed var(--error);outline-offset:-4px;background:var(--error-bg)}
      ...
      .sched-job-daycell .jobcell-add{position:static;transform:none;opacity:0.5;width:100%;height:28px;border-radius:8px;border:1px dashed var(--border);background:none;color:var(--text-secondary);font-size:1.1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;margin-top:auto;transition:all 0.15s}
      .sched-job-daycell:hover .jobcell-add{opacity:0.8}
      .sched-job-daycell .jobcell-add:hover{opacity:1;border-color:var(--accent);color:var(--accent);background:var(--card-hover);transform:none}
      ...
      .sched-jobtile .jt-top-right{display:flex;align-items:center;gap:0.35rem;flex-shrink:0}
      .sched-jobtile .jt-bell{background:none;border:none;cursor:pointer;font-size:0.75rem;padding:0;line-height:1}
      .sched-jobtile .jt-bell:hover{transform:scale(1.15)}
      ...
      .sched-job-card[draggable="true"]:active{outline:2px dashed var(--accent);outline-offset:1px}
      .sched-jobtile:active{outline:2px dashed var(--accent);outline-offset:1px}
      .sched-rail-chip:active{outline:2px dashed var(--accent);outline-offset:1px}
      .sched-strip-job:active{outline:2px dashed var(--accent);outline-offset:1px}
      ...
      .sched-job-card .job-num{font-weight:700;font-family:'JetBrains Mono',monospace;font-size:0.7rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}
      .sched-job-card .job-top{min-width:0}
      ...
      .sm-assign-left .a-label{font-weight:700;font-family:'JetBrains Mono',monospace;font-size:0.8rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}
  ...
  function legendBar() {
    return `<div class="sched-legend">
      <div class="sched-legend-items">
        <span class="lg-item"><span class="lg-dot" style="background:var(--error)"></span> Not assigned</span>
        <span class="lg-item"><span class="lg-dot" style="background:#f59e0b"></span> Assigned, not notified</span>
        <span class="lg-item"><span class="lg-dot" style="background:var(--success)"></span> Assigned &amp; notified</span>
        <span class="lg-item"><span class="lg-dot" style="background:#3b82f6"></span> Change notified</span>
      </div>
      <button class="btn-primary" id="dt-notify-all" style="padding:0.5rem 1rem;font-size:0.8rem;white-space:nowrap">🔔 Notify All — Next Week Live</button>
    </div>`;
  }
  ...
  function bindDesktop(container) {
    ...
    dt.querySelector('#dt-notify-all')?.addEventListener('click',ev=>notifyAllNextWeek(ev,container));
  ...
  /* ═══════════════════════════════════════ JOB VIEW ═══════════════════════════════════════ */
  function groupKeyForAssignment(a) { return (a.jobNumber || a.siteName || '?'); }
  function assignmentActiveOnDate(a, dateKey) {
    if (a.schedule === 'oneoff') return a.startDate === dateKey;
    return dateKey >= a.startDate && (!a.endDate || dateKey <= a.endDate) && isWeekday(dateKey) && !(a.skipDates||[]).includes(dateKey);
  }
  ...
  function getJobGroupsForDate(dateKey) {
    ...
    for (const a of assignments) {
      ...
      if (!assignmentActiveOnDate(a, dateKey)) continue;
      const gk = groupKeyForAssignment(a);
      const key = gk + '|' + dateKey;
      if (!groups[key]) groups[key] = { key, groupKey: gk, jobNumber: a.jobNumber||'', siteName: a.siteName||'', clientName: a.clientName||'', dateKey, people: [] };
      groups[key].people.push(a);
    }
    return Object.values(groups);
  }

  // When an already-notified assignment is edited, flag it so re-notify shows blue
  function flagEdited(a) { if (a.notified) { a.wasNotified = true; } a.notified = false; }

  // Status colours. green=first-time notified. blue=re-notified after an edit. amber=assigned not notified. red=none.
  function jobTileStatus(group) {
    if (!group.people.length) return 'red';
    if (!group.people.every(p => p.notified)) return 'amber';
    return group.people.some(p => p.wasNotified) ? 'blue' : 'green';
  }
  function cardStatus(a) {
    if (!a.notified) return 'amber';
    return a.wasNotified ? 'blue' : 'green';
  }
  ...
  function buildJobView(viewToggle, days, dayNames, weekEnd) {
    return `
      ...
      ${legendBar()}
      <div class="sched-grid-wrap"><div class="sched-grid sched-jobgrid" style="grid-template-columns:repeat(${days.length},1fr);">
        ${days.map(d=>`<div class="sched-col-head ${isToday(d)?'today':''}">${dayNames[d.getDay()===0?6:d.getDay()-1]}<br>${formatDate(d)}</div>`).join('')}
        ${days.map(d=>{const dk=formatDateKey(d);const groups=getJobGroupsForDate(dk);return`
          <div class="sched-job-daycell ${isToday(d)?'today-col':''}" data-date="${dk}">
            ${groups.map(g=>{const st=jobTileStatus(g);const rep=g.people[0];const anyUn=g.people.some(p=>!p.notified);const times=rep?getAssignmentTimes(rep):'';const note=g.people.find(p=>p.notes)?.notes||'';return`
            ...
            <button class="cell-add jobcell-add" data-jobcell-date="${dk}" title="Add job to this day">+</button>
          </div>`;}).join('')}
      </div></div>`;
  }
  ...
  function bindJobView(dt, container) {
    ...
    dt.querySelector('#dt-today')?.addEventListener('click',()=>{currentWeekStart=getMonday(new Date());rerender(container);});
    dt.querySelector('#dt-weekends')?.addEventListener('change',e=>{showWeekends=e.target.checked;rerender(container);});
    // + button on each day column → pick a job/site, then staff it
    dt.querySelectorAll('.jobcell-add').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();openNewJobModal(container, b.dataset.jobcellDate);}));
    // Tile click / edit → staff modal
    ...
    dt.querySelectorAll('[data-notify-tile]').forEach(b=>b.addEventListener('click',async e=>{
      ...
      const people=assignments.filter(a=>(a.type==='job'||a.type==='site')&&groupKeyForAssignment(a)===gk&&assignmentActiveOnDate(a,dk)&&!a.notified);
  ...
  // Job view: + on a day → pick job or site → open staffing for it
  function openNewJobModal(container, dateKey) {
    let tab='job',jobQuery='',siteQuery='';
    ...
    function rm(){
      ...
      overlay.innerHTML=`<div class="sched-modal">
        <button class="sched-modal-close" id="nj-close">×</button>
        <h3>Add to ${formatDateFull(parseDateKey(dateKey))}</h3>
        <div class="sched-modal-tabs">
          <button class="sched-modal-tab ${tab==='job'?'active':''}" data-njtab="job">Job Number</button>
          <button class="sched-modal-tab ${tab==='site'?'active':''}" data-njtab="site">Site</button>
        </div>
        ${tab==='job'?`
          <label>Search Job</label>
          <input class="sched-input" id="nj-job-search" type="text" placeholder="Search job number, client or site…" value="${jobQuery}" autofocus>
          <div class="sched-job-results">${fJobs.length===0?`<div class="sched-empty">${jobs.length===0?'No jobs found':'No matching jobs'}</div>`:''}
            ${fJobs.map(j=>`<div class="sched-job-result" data-nj-job="${j.number}"><div><span class="jr-num">${j.number}</span> <span class="jr-client">${j.client}${j.site?' · '+j.site:''}</span></div></div>`).join('')}</div>
        `:`
          <label>Search Site</label>
          <input class="sched-input" id="nj-site-search" type="text" placeholder="Search site name, client or address…" value="${siteQuery}" autofocus>
          <div class="sched-job-results">${fSites.length===0?`<div class="sched-empty">${sites.length===0?'No sites found':'No matching sites'}</div>`:''}
            ${fSites.map(s=>`<div class="sched-site-result" data-nj-site="${s.id}"><span class="sr-name">${s.name}</span><span class="sr-client">${s.clientName}</span></div>`).join('')}</div>
        `}
        <div class="sched-modal-actions"><button class="btn-secondary" id="nj-cancel">Cancel</button></div>
      </div>`;
      overlay.querySelector('#nj-close')?.addEventListener('click',close);
      overlay.querySelector('#nj-cancel')?.addEventListener('click',close);
      overlay.querySelectorAll('[data-njtab]').forEach(t=>t.addEventListener('click',()=>{tab=t.dataset.njtab;jobQuery='';siteQuery='';rm();}));
      if(tab==='job'){
        overlay.querySelector('#nj-job-search')?.addEventListener('input',e=>{jobQuery=e.target.value;const p=e.target.selectionStart;rm();const i=overlay.querySelector('#nj-job-search');if(i){i.value=jobQuery;i.focus();i.setSelectionRange(p,p);}});
        overlay.querySelectorAll('[data-nj-job]').forEach(el=>el.addEventListener('click',()=>{const job=jobs.find(j=>j.number===el.dataset.njJob);close();if(job)openStaffModal(container,job,dateKey);}));
      } else {
        overlay.querySelector('#nj-site-search')?.addEventListener('input',e=>{siteQuery=e.target.value;const p=e.target.selectionStart;rm();const i=overlay.querySelector('#nj-site-search');if(i){i.value=siteQuery;i.focus();i.setSelectionRange(p,p);}});
        overlay.querySelectorAll('[data-nj-site]').forEach(el=>el.addEventListener('click',()=>{const s=sites.find(x=>x.id===el.dataset.njSite);close();if(s)openStaffModal(container,{number:'',site:s.name,client:s.clientName,groupKey:s.name,siteId:s.id},dateKey);}));
      }
    }
  ...
  function openStaffModal(container, job, dateKey) {
    ...
    function rm(){
      ...
      overlay.querySelector('#sm-apply-times')?.addEventListener('click',async()=>{
        ...
        for(const a of existing()){a.startTime=st;a.endTime=et;flagEdited(a);await DB.saveAssignment(a);}
        rm();
      });
      ...
      overlay.querySelector('#sm-apply-note')?.addEventListener('click',async()=>{
        ...
        for(const a of existing()){a.notes=nt;flagEdited(a);await DB.saveAssignment(a);}
        rm();
      });
      // Per-person time overrides (save on change)
      overlay.querySelectorAll('[data-ptime-start]').forEach(inp=>inp.addEventListener('change',async e=>{const a=assignments.find(x=>x.id===inp.dataset.ptimeStart);if(!a)return;a.startTime=e.target.value||null;flagEdited(a);await DB.saveAssignment(a);rm();}));
      overlay.querySelectorAll('[data-ptime-end]').forEach(inp=>inp.addEventListener('change',async e=>{const a=assignments.find(x=>x.id===inp.dataset.ptimeEnd);if(!a)return;a.endTime=e.target.value||null;flagEdited(a);await DB.saveAssignment(a);rm();}));
            ...
            <div class="sm-emp-body"><div class="sm-emp-body-inner">
              ${ea.map(a=>{const lbl=getAssignmentLabel(a);const sub=getAssignmentSub(a);const bc=getAssignmentBorderColor(a);const isSite=a.type==='site';const isLeave=a.type==='leave';const isWorkshop=a.type==='workshop';const isLinked=a.linked;const isIndef=a.schedule==='indefinite'&&!a.endDate;
                return`<div class="sm-assign-card ${isSite?'is-site':isLeave?'is-leave':isWorkshop?'is-workshop':'is-job'}" ${(isLeave||isWorkshop)?`style="border-left-color:${bc}"`:''}><div class="sm-assign-left">
                  ...
                  <div class="sm-assign-actions">${a.rdoAuto?'<span class="sm-roster-lock" title="From RDO roster">🔒</span>':`<button class="sm-assign-btn" data-editnote="${a.id}" title="Edit times & notes" style="font-size:0.7rem">✎</button><button class="sm-assign-btn" data-notify="${a.id}" title="Notify" style="font-size:0.7rem">🔔</button>${isIndef&&isLinked?`<button class="sm-assign-btn end-btn" data-end="${a.id}" data-end-date="${dk}" title="End here">⏹</button>`:''}<button class="sm-assign-btn" data-remove="${a.id}" data-remove-date="${dk}" data-remove-linked="${isLinked}" title="Remove">×</button>`}</div></div>`;}).join('')}
  ...
  function bindCardActions(root,container) {
    ...
    root.querySelectorAll('[data-editnote]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();const a=assignments.find(x=>x.id===b.dataset.editnote);if(a)openNotesEditor(container,a);}));
  }

  function openNotesEditor(container, a) {
    const overlay=document.createElement('div');overlay.className='sched-confirm-overlay';
    overlay.innerHTML=`<div class="sched-confirm" style="text-align:left;max-width:420px">
      <p style="font-weight:700;margin-bottom:0.25rem">Edit — ${a.employeeName}</p>
      <p style="font-size:0.8rem;color:var(--text-secondary);margin-bottom:0.75rem">${getAssignmentLabel(a)} · ${a.startDate}</p>
      <label style="display:block;font-size:0.8rem;font-weight:600;color:var(--text-secondary);margin-bottom:0.3rem">Times <span style="font-weight:400">(optional)</span></label>
      <div class="sched-time-row" style="margin-bottom:0.5rem">
        <div style="flex:1"><span class="sched-time-lbl">Start</span><input class="sched-input" id="ne-start" type="time" value="${a.startTime||''}" style="width:100%"></div>
        <div style="flex:1"><span class="sched-time-lbl">End</span><input class="sched-input" id="ne-end" type="time" value="${a.endTime||''}" style="width:100%"></div>
      </div>
      <label style="display:block;font-size:0.8rem;font-weight:600;color:var(--text-secondary);margin-bottom:0.3rem">Location</label>
      <div class="sched-loc-toggle" style="margin-bottom:0.5rem">
        <button class="sched-loc-btn ${(a.location||'site')==='site'?'active':''}" data-ne-loc="site">On Site</button>
        <button class="sched-loc-btn ${a.location==='workshop'?'active':''}" data-ne-loc="workshop">Workshop/Office</button>
      </div>
      <label style="display:block;font-size:0.8rem;font-weight:600;color:var(--text-secondary);margin-bottom:0.3rem">Additional Notes <span style="font-weight:400">(visible to employee)</span></label>
      <textarea class="sched-input" id="ne-msg" rows="3" style="width:100%;resize:vertical;font-size:16px" placeholder="Additional notes visible to the employee…">${a.notes||''}</textarea>
      <div class="sched-confirm-actions" style="margin-top:1rem"><button class="btn-secondary" id="ne-cancel">Cancel</button><button class="btn-primary" id="ne-save">Save</button></div>
    </div>`;
    const inner=overlay.querySelector('.sched-confirm');
    inner.addEventListener('mousedown',ev=>ev.stopPropagation());
    let locVal=a.location||'site';
    overlay.querySelectorAll('[data-ne-loc]').forEach(b=>b.addEventListener('click',()=>{locVal=b.dataset.neLoc;overlay.querySelectorAll('[data-ne-loc]').forEach(x=>x.classList.toggle('active',x.dataset.neLoc===locVal));}));
    ...
    overlay.querySelector('#ne-save').addEventListener('click',async()=>{
      a.startTime=overlay.querySelector('#ne-start')?.value||null;
      a.endTime=overlay.querySelector('#ne-end')?.value||null;
      a.location=locVal;
      a.notes=overlay.querySelector('#ne-msg')?.value||'';
      flagEdited(a); // details changed → needs re-notify (blue after)
    ...
    document.body.appendChild(overlay);
    overlay.querySelector('#ne-start')?.focus();
  }
  ...
  // Notify everyone who has any assignment in the visible week that the schedule is live
  async function notifyAllNextWeek(ev, container){
    const btn=ev?.target;
    const days=getDaysOfWeek();const keys=new Set(days.map(d=>formatDateKey(d)));
    // Everyone with an assignment this week
    const names=new Set();
    for(const a of assignments){
      let inWeek=false;
      if(a.schedule==='oneoff')inWeek=keys.has(a.startDate);
      else inWeek=[...keys].some(k=>k>=a.startDate&&(!a.endDate||k<=a.endDate)&&isWeekday(k)&&!(a.skipDates||[]).includes(k));
      if(inWeek)names.add(a.employeeName);
    }
    if(!names.size){showConfirm('No one has an assignment this week to notify.',()=>{});return;}
    showConfirm(`Send "<strong>Next Week's Schedule is now live</strong>" to all ${names.size} scheduled ${names.size===1?'employee':'employees'}?`, async ()=>{
      if(btn){btn.disabled=true;btn.textContent='Sending…';}
      const msg="Next Week's Schedule is now live";
      for(const name of names){
        try{const sb=await DB.init();await sb.from('notifications').insert({employee_name:name,message:msg,type:'schedule_live'});await sb.functions.invoke('send-notification',{body:{employee_name:name,message:msg}});}catch(err){console.warn('notify all failed for',name,err);}
      }
      // Mark all their week assignments notified
      for(const a of assignments){
        if(!names.has(a.employeeName))continue;
        let inWeek=false;
        if(a.schedule==='oneoff')inWeek=keys.has(a.startDate);
        else inWeek=[...keys].some(k=>k>=a.startDate&&(!a.endDate||k<=a.endDate));
        if(inWeek&&!a.notified){a.notified=true;await DB.markNotified(a.id);}
      }
      rerender(container);
    });
  }
    ...
    function rm() {
      ...
      overlay.innerHTML=`<div class="sched-modal">
        ...
        <div class="sched-modal-tabs">
          ...
          <button class="sched-modal-tab ${activeTab==='pinned'?'active':''}" data-tab="pinned">★ Favourites</button>
        ...
        `:activeTab==='pinned'?`
          ...
          <div class="sched-job-results">${pinnedSites.length===0?'<div class="sched-empty">No favourites yet — tap the ☆ on a site in the Sites tab</div>':''}
            ${pinnedSites.map(s=>`<div class="sched-site-result ${selectedSite?.id===s.id?'selected':''}" data-sid="${s.id}"><div style="display:flex;justify-content:space-between;align-items:center"><div><span class="sr-name">${s.name}</span><span class="sr-client">${s.clientName}</span></div><button class="sched-pin-btn pinned" data-unpin="${s.id}" title="Remove favourite">★</button></div></div>`).join('')}</div>
